...
class SlotAttention(nn.Module):
    ...
    def __init__(self, device, num_slots, encoder_dims, iters=3, hidden_dim=128, pos_embed=False, eps=1e-8):
        ...
        if pos_embed:
            self.position_embedding = nn.Parameter(torch.randn(self.num_slots, encoder_dims))
        ...
        self.slots_embedding = nn.Embedding(self.num_slots, encoder_dims)
    ...
    def forward(self, inputs, return_attn=False):
        ...
        if self.position_embedding is not None:
            slots = slots + self.position_embedding
        ...
        for _ in range(self.iters):
            ...
            attn = attn_ori / attn_ori.sum(dim=-1, keepdim=True)  # weighted mean.
