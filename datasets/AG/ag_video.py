...
class AGVideoDataset(TvCocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        for img_id in img_ids:
            ...
            video_name = path.split('/')[-2].split('.')[0]
            target = {'video_id': video_name, 'image_id': img_id, 'annotations': target}
...
def build(image_set, args):
    ...
    dataset = AGVideoDataset(img_folder, anno_file, transforms=make_coco_transforms(image_set, args.image_size), cache_mode=args.cache_mode, local_rank=get_local_rank(), local_size=get_local_size(), is_train=(not args.eval))
